...
//	parms2.theta = 0;
//
//	cusimann_minimum[0] = 0.5;
//
//	double f_nelderMead_minimum;
//		double *nelderMead_minimum = (double*)malloc(n*sizeof(double));
//		nelderMead_optimize(n, lb, ub, cusimann_minimum, f_nelderMead, &parms2, nelderMead_minimum, &f_nelderMead_minimum);
//
...
//		return 0;
	...
	cusimann_optimize(n_threads_per_block, n_blocks, T_0, T_min, N, rho, n, lb, ub, Energy<real>(), d_parms, cusimann_minimum, &f_cusimann_minimum);
...
public:
	__host__ __device__ T operator() (const T *x, unsigned int n, void *f_data) const
	{
		...
		int i;
		for(i=0;i<n-1;i++)
			f_x = f_x + aux*sqrt( pow(x[i],2) + pow(x[i+1],2) ) + 3.0f*( cos(2.0f*x[i]) + sin(2.0f*x[i+1]) );
...
double f_nelderMead2(unsigned int n, const double *x, double *grad, void *f_data){
...
int main2() {
	...
	cusimann_optimize(n_threads_per_block, n_blocks, T_0, T_min, N, rho, n, lb, ub, Ackley<real>(), NULL, cusimann_minimum, &f_cusimann_minimum);
